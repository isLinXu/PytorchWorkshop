...
def scale_img(img, ratio=1.0, same_shape=True):  # img(16,3,256,416), r=ratio
    ...
    if not same_shape:  # pad/crop img
        ...
        h, w = [math.ceil(x * ratio / gs) * gs for x in (h, w)]  # 如果形状不同，图片长宽ModelEMA变成64的整数倍
    return F.pad(img, [0, w - s[1], 0, h - s[0]], value=0.447)  # value = imagenet mean TODO: 这里的pad有点厉害


class ModelEMA:  # 指数移动平均模型
    ...
    def __init__(self, model, decay=0.9999, device=''):
        ...
        self.ema.eval()  # TODO:eval()的作用到底是什么，将模型参数分离出来？？
        ...
        with torch.no_grad():
            ...
            for k, v in esd.items():
                if v.dtype.is_floating_point:
                    v *= d
                    v += (1. - d) * msd[k].detach()  # TODO detach()

    def update_attr(self, model):
        # Assign attributes (which may change during training)
        for k in model.__dict__.keys():
            if not k.startswith('_'):
                setattr(self.ema, k, getattr(model, k))  # 将
                # setattr函数与getattr函数对应。
                # setattr()语法:
                # setattr(object, name, value): object---对象; name---字符串,对象属性(key); value---属性值
                # 例:class A:
                #        bar = 1
                # >>>a = A()
                # >>>getattr(a, 'bar') ---> 1
