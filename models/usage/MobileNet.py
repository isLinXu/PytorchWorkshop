...
__all__ = ['MobileNetV2','MobileNetV2_bot']
...
class ConvBlock(nn.Module):
	...
	def __init__(self, in_c, out_c, k, s=1, p=0, g=1):
		...
		self.conv = nn.Conv2d(in_c, out_c, k, stride=s, padding=p, bias=False, groups=g) #k是kernel size
...
class MobileNetV2_bot(nn.Module):
	"""MobileNetV2
	...
	def __init__(self, classes=2, loss={'xent'}, **kwargs):
		super(MobileNetV2_bot, self).__init__()
		self.loss = loss
		...
		self.gender_classifier = nn.Linear(1280, classes)
		self.staff_classifier = nn.Linear(1280, classes)
		self.customer_classifier = nn.Linear(1280, classes)
		self.stand_classifier = nn.Linear(1280, classes)
		self.sit_classifier = nn.Linear(1280, classes)
		self.phone_classifier = nn.Linear(1280, classes)
